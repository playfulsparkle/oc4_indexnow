...
$_['text_troubleshot']                    = '<ul>
    <li><strong>The submitted URL list contains invalid URLs or URLs with a host that does not match the active store:</strong> Ensure that the URL list text field contains valid URLs and that the host (domain) of the URLs matches the active store’s domain.</li>
    <li><strong>Could not download sitemap file xyz:</strong> Ensure that the sitemap URL is entered correctly.</li>
    <li><strong>Invalid file type. Please upload a valid file:</strong> Ensure you are uploading a valid XML sitemap file.</li>
    <li><strong>Queue list is not populated:</strong> Ensure that you enable the following options: Categories, Products, Manufacturers, Information Pages, Article Topics, and Articles on your site are added, updated, or deleted. Also, make sure that each section is enabled, and the correct store is selected when editing products, categories, etc.</li>
    <li><strong>No services are enabled. Please enable at least one service:</strong> You must enable at least one IndexNow service before clicking the "Submit Queued URLs" button.</li>
</ul>';
